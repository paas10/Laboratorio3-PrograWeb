...
function validarTexto(num) {
  ...
  for (var i = num; i < letterArray.length; i++) {
    ...
    if (letterArray[i] == '{' || letterArray[i] == '[' || letterArray[i] == '(') {
      letterArray[i] == '{' ? (curlyBrackets = true) : (letterArray[i] == '[' ? squareBrackets = true : parentesis = true);
      i = validarTexto(++i);
    } else if (letterArray[i] == '}' || letterArray[i] == ']' || letterArray[i] == ')') {
    ...
    try {
      ...
      if ((letterArray[i] == '}' && !curlyBrackets) || (letterArray[i] == ']' && !squareBrackets) || (letterArray[i] == ')' && !parentesis)) {
        ...
        letterArray[i] == '{' ? (curlyBrackets = false) : (letterArray[i] == '[' ? squareBrackets = false : parentesis = false);
      }
    } catch(e) {
      break;
    }
